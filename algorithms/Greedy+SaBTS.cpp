...
int *setFlag, *globalsetFlag;   	  //labeled nodes belong to which sets, 0-S, 1-V\S
...
//parameters (5)
int alfa = 100;                       //tabu tenure management factor = 100
int D = 6000;                         //depth of tabu search = 6000
int T = 1000;                         //stagnation threshold = 1000
double L0 = 0.4;                      //initial jump magnitude = 0.4 * |V|
double P0 = 0.8;                      //minimum probability = 0.8
...
void generate_initialize(int*, long long int&);
void generate_elite_sol();
...
/* initialization for generate_elite_sol() */
void generate_initialize(int *set, long long int &sol)
{
	//randomly choose a vertex from V adding to the set S
	//initial solution: S = {v}, V\S = {rest nodes}
	long long int v = (long long int)rand()%nbNodes;
...
/* generate elite initial sol */
void generate_elite_sol()
{
	long long int add_pos, add_node;
	long long int eval_degreeS, eval_degreeVS, eval_cG;
	long long int eval_sol, best_sol;
	long long int adding_pos[10000], adding_node[10000];
	int nb_best_sol, index;
	long long int sol, initial_best;
	int *set, *initial_set;
	set = new int[nbNodes];
	initial_set = new int[nbNodes];

	initial_best = MAX_INT;
	for (int nb_sol = 0; nb_sol < 10; nb_sol++)
	{
		generate_initialize(set, sol);
		...
		while(1)
		{
			//find(check) the best ADD move
			for (long long int k = 0; k < sizeVS; k++)
			{
				//choose the evaluate node
				add_pos = (long long int) rand() % sizeVS;
				add_node = setVS[add_pos];
				...
				{
					best_sol = eval_sol;
					adding_pos[0] = add_pos;
					adding_node[0] = add_node;
					nb_best_sol = 1;
				}
				else if ((eval_sol == best_sol) && (nb_best_sol < 10000))
				{
					adding_pos[nb_best_sol] = add_pos;
					adding_node[nb_best_sol] = add_node;
					nb_best_sol++;
				}
			}
			...
			if ((best_sol < sol) && (nb_best_sol > 0))
			{
				...
				add_pos = adding_pos[index];
				add_node = adding_node[index];
		...
		printf("%d - initial_best = %.10lf\n", nb_sol, (double) initial_best / (double) MAX_INT);

		if (initial_best < localBestSol)
		{
			localBestSol = initial_best;
			for (long long int i = 0; i < nbNodes; i++)
				setFlag[i] = initial_set[i];
		}
...
//	int count = 0;
	...
	/* PART1 generate elite initial solution */
	generate_elite_sol();
	sol = localBestSol;
...
int main(int argc, char* argv[])
{
	...
	else
	{
		...
		printf("<instanceName> <dataSet> <runTime> <nbRepeat> <alfa> <D> <T> <L0> <P0>\n");
		exit(-1);
	}
